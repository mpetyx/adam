...
class Stemmer
{
    ...
    function stem( $word )
    {
        ...
        $result = $first . $word;
        if ($result[strlen($result)-1]=="'" ||$result[strlen($result)-1]=="." ||$result[strlen($result)-1]=="-" ||$result[strlen($result)-1]=="_"
		)
		$result=$this->stem(substr($result,0,-1));
		else if ($result[0]=="'" ||$result[0]=="." ||$result[0]=="-" ||$result[0]=="_"
		)$result=$this->stem(substr($result,1));
        return $result;
    }
